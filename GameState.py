...
class GameState:
    ...
    # send the game state json to remote host
    def send_encrypted_text(self, remote_socket, key):
        success = True
        plaintext = self._get_data_plain_text()
        ...
        encoded_plaintext = plaintext.encode('utf8')
        padded_plaintext = pad(encoded_plaintext, AES.block_size)
        encrypted_message = cipher.encrypt(padded_plaintext)
        b64encrypted_message = base64.b64encode(cipher.iv + encrypted_message)
        
        # ice_print_debug(f"Sending message to client: {plaintext} (Unencrypted)")
        # send len followed by '_' followed by cypher
        m = str(len(b64encrypted_message))+'_'

        try:
            remote_socket.sendall(m.encode("utf-8"))
            remote_socket.sendall(b64encrypted_message)
        except OSError:
            print("Connection terminated")
            success = False
        return success
